...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys }) => {
  ...
  const handleDownload = () => {
    if (displayImageUrl) {
      downloadImage(displayImageUrl, `look-${Date.now()}.png`);
    }
  };
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      <div className="absolute top-4 left-4 right-4 z-30 flex justify-between items-center pointer-events-none">
        <button 
            onClick={onStartOver}
            className="pointer-events-auto flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 text-sm backdrop-blur-sm shadow-sm"
        >
        ...
        {displayImageUrl && !isLoading && (
          <button 
              ...
              className="pointer-events-auto flex items-center justify-center text-center bg-gray-900 border border-gray-800 text-white font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-gray-800 active:scale-95 text-sm shadow-md"
          >
